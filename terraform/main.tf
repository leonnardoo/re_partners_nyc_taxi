...
resource "google_storage_bucket" "landing_layer" {
  ...
  lifecycle_rule {
    condition {
      ...
      matches_prefix = ["nyc_taxi/"] # Applies only to this path.
    }
    action {
      type = "Delete"
    }
...
resource "google_storage_bucket_object" "nyc_taxi_folder_structure" {
  name    = "nyc_taxi/"
  content = " " # Empty content to initialize the "folder"
  bucket  = google_storage_bucket.landing_layer.name
}
...
# Service Account for Pipeline (Landing)
resource "google_service_account" "landing_pipeline_sa" {
  account_id   = var.landing_pipeline_sa
  display_name = "Service Account for Data Processing (Landing)"
}
...
resource "google_storage_bucket_iam_member" "landing_admin" {
  ...
  member = "serviceAccount:${google_service_account.landing_pipeline_sa.email}"
}
...
resource "google_bigquery_dataset" "gold_nyc_taxi" {
  dataset_id    = "gold_nyc_taxi"
  friendly_name = "Gold layer dataset for NYC Taxi Trips"
  description   = "This dataset was provided by Terraform"
  location      = var.region
  ...
  # Recommended to prevent accidental deletion in production.
  lifecycle {
